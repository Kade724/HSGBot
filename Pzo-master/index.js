...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'PZO\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=558597940209:+55 27997179500\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["558597940209@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"558597940209@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["558597940209@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["558597940209@s.whatsapp.net"]//ubah nomor lo
			const premium = ["558597940209@s.whatsapp.net"] 
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("thay")){
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
				...
				if (messagesC.includes("pzo")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/pzof.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS 𝑷𝒁𝑶 BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Pzo domina vida
          ...
          reply('Espere..')
				...
				case 'f':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie fotos com legendas *.pfig* ou marque uma imagem que já foi enviada`)
					}
					break
				case 'shadow': 
				if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Pzo ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = '╭────「 *PREMIUM👑* 」──*\n│+ *Número* : \n│+ *Expirado*: *30 Days*\n│+ *Status*: *ATIVO*\n│ Thx por atualizar para premium🥰\n*╰──────「 *posição* 」────'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT HSG ✨* 」\n*│+ wa.me/558597940209*╰──────*「 *PZO* 」*────*\n\n*_SE QUER SER ADMIN DO BOT PZO_*\n*_Tipo /iklan_*' })
					break
					case 'txtfig':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/558597940209/*╰──────*「 *HSG* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO PZO BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
